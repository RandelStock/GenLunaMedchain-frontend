...
export default function MedicineList() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Extract unique barangays from medicines
  ...
  // Also derive role from backend user profile for reliability
  useEffect(() => {
    const fetchMe = async () => {
      try {
        ...
        if (barangay) {
          ...
          // If not municipal staff/admin, automatically filter by their barangay
  ...
  const filteredAndSortedMedicines = medicines
    .filter((med) => {
      ...
      // Check if medicine is expired based on stock expiry dates
    ...
    .sort((a, b) => {
      // Sort by earliest expiry date in stocks
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
          <div className="sticky top-0 bg-white border-b border-gray-200 p-6 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <button 
                onClick={async () => {
                  await fetchData();
                  // Update selected medicine with fresh data
          ...
          <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {selectedMedicine.barangay && (
                  <div className="col-span-2 bg-blue-50 p-4 rounded-lg border-l-4 border-blue-500">
                    <label className="text-sm font-medium text-blue-700">Barangay</label>
                    <p className="text-lg font-semibold text-blue-900 mt-1">{selectedMedicine.barangay}</p>
                  </div>
                )}
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  <label className="text-sm font-medium text-gray-500">Medicine Type</label>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-xl font-bold text-blue-600">{totalStock} units total</div>
              </div>
                      ...
                      <div key={idx} className="bg-gray-50 p-4 rounded-lg border-l-4 border-blue-500">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div>
                            ...
                            <p className="text-sm font-semibold text-gray-900 mt-1">{stock.batch_number}</p>
                          </div>
                          <div>
                            <label className="text-xs font-medium text-gray-500">Quantity</label>
                            <p className="text-sm text-gray-900 mt-1">{stock.remaining_quantity} / {stock.quantity}</p>
                          </div>
                          <div>
                            <label className="text-xs font-medium text-gray-500">Expiry Date</label>
                            <p className="text-sm text-gray-900 mt-1">{new Date(stock.expiry_date).toLocaleDateString()}</p>
                          </div>
                          <div>
                          ...
                          {stock.storage_location && (
                            <div className="col-span-2 md:col-span-4">
                              <label className="text-xs font-medium text-gray-500">Storage Location</label>
          ...
          <div className="sticky bottom-0 bg-gray-50 border-t border-gray-200 p-4 flex justify-end gap-2">
            <button onClick={() => setShowViewModal(false)} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">
              Close
            </button>
            {(isAdmin || isStaff) && (
              <>
  ...
  const EditModal = () => {
    ...
    try {
      // Step 1: Update in your backend database first
      const medicineData = {
      ...
      // Step 2: Create hash of updated data
      ...
      // Step 3: Update hash on blockchain
  ...
  const DeleteModal = () => {
    ...
    try {
      // Step 1: Delete from blockchain first
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto p-6">
        <div className="mb-6">
          ...
          <p className="text-gray-600">Manage and track your medicine stock</p>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-mono bg-gray-100 px-3 py-1 rounded-md text-gray-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 text-gray-900"
              />
            </div>

            {/* Barangay/RHU Filter - Visible to all users */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Filter by Barangay / RHU</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Sort by Expiry</label>
              <select
                value={sortOrder}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <div className="text-2xl font-bold text-gray-900">{filteredAndSortedMedicines.length}</div>
          </div>
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div className="text-sm font-medium text-gray-600 mb-1">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center text-red-600">
            {error}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {paginatedMedicines.map((med) => {
                ...
                return (
                  <div
                    key={med.medicine_id}
                    className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 p-4 flex flex-col justify-between"
                  >
                    <div>
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          >
                            {hasExpiredStock ? "Expired Batch" : "Active"}
                          </span>
                          {med.barangay && (
                      ...
                      <div className="text-xs text-gray-500 space-y-1">
                        ...
                        <p>Manufacturer: {med.manufacturer || "N/A"}</p>
                        <p>Type: {med.medicine_type || "N/A"}</p>
                      </div>
                    </div>
                      ...
                      {(isAdmin || isStaff) && (
                        <>
                          <button
                            ...
                            className="px-3 py-1 bg-yellow-50 text-yellow-700 rounded-md text-sm hover:bg-yellow-100"
                          >
                            Edit
                          </button>
                          <button
