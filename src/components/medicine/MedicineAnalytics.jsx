...
const MedicineAnalytics = () => {
  ...
  const calculateAnalytics = (medicines, allStocks) => {
    ...
    medicines.forEach(med => {
      (med.medicine_stocks || []).forEach(stock => {
        ...
        if (expiry > now && expiry <= expiryDate && stock.remaining_quantity > 0) {
          expiryAlerts.push({
    ...
    // Recently added medicines (in selected time range)
    const recentlyAddedMedicines = allStocks
      .filter(stock => new Date(stock.date_received) >= startDate)
      .map(stock => {
    ...
    // Top restocked medicines (by quantity added in time range)
    const restockMap = {};
    allStocks
      .filter(stock => new Date(stock.date_received) >= startDate)
      .forEach(stock => {
        ...
        restockMap[name].total_quantity += stock.quantity;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600">Insights and trends for medicine inventory</p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-6 border border-blue-200 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs font-semibold text-blue-800 bg-blue-200 px-2 py-1 rounded">TOTAL</span>
          </div>
          <div className="text-3xl font-bold text-gray-900 mb-1">{analytics.totalMedicines}</div>
          <div className="text-sm font-medium text-gray-700">Total Medicines</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-6 border border-green-200 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs font-semibold text-green-800 bg-green-200 px-2 py-1 rounded">STOCK</span>
          </div>
          <div className="text-3xl font-bold text-gray-900 mb-1">{analytics.totalStock.toLocaleString()}</div>
          <div className="text-sm font-medium text-gray-700">Total Units in Stock</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-xl p-6 border border-yellow-200 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs font-semibold text-yellow-800 bg-yellow-200 px-2 py-1 rounded">LOW</span>
          </div>
          <div className="text-3xl font-bold text-gray-900 mb-1">{analytics.lowStockCount}</div>
          <div className="text-sm font-medium text-gray-700">Low Stock Medicines</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-6 border border-red-200 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs font-semibold text-red-800 bg-red-200 px-2 py-1 rounded">ALERT</span>
          </div>
          <div className="text-3xl font-bold text-gray-900 mb-1">{analytics.expiringSoonCount}</div>
          <div className="text-sm font-medium text-gray-700">Expiring Soon (30 days)</div>
        </div>
      </div>

      {/* Top Restocked Medicines & Recently Added */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* Top Restocked */}
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex items-center gap-3 mb-5">
            <div className="bg-purple-100 p-3 rounded-xl">
              <FaTrendingUp className="text-xl text-purple-600" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-gray-900">Top Restocked Medicines</h3>
              <p className="text-xs text-gray-600">Most added in selected period</p>
            </div>
          </div>
                ...
                <div key={idx} className="flex items-center justify-between p-3 bg-purple-50 rounded-lg border border-purple-200">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">{med.restock_count} restock{med.restock_count > 1 ? 's' : ''}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-lg font-bold text-purple-600">+{med.total_quantity}</span>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">Latest additions in selected period</p>
            </div>
          </div>
          ...
          {analytics.recentlyAddedMedicines.length > 0 ? (
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {analytics.recentlyAddedMedicines.map((stock, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200 hover:bg-green-100 transition-colors">
                  <div className="flex-1">
                    <p className="font-semibold text-gray-900 text-sm">{stock.medicine_name}</p>
                    <p className="text-xs text-gray-600">Batch: {stock.batch_number}</p>
                  </div>
                  <div className="text-right mr-3">
                    <span className="text-base font-bold text-green-600">+{stock.quantity}</span>
                  </div>
      ...
      // ========== PART 2 OF 2 - CONTINUATION OF MEDICINEANALYTICS.JSX ==========

      {/* Expiry Alerts & Category Distribution */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* Expiry Alerts */}
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">Medicines expiring within 30 days</p>
            </div>
          </div>
              ...
              {analytics.expiryAlerts.map((alert, idx) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <p className="font-semibold text-gray-900 text-sm">{alert.medicine_name}</p>
                      <p className="text-xs text-gray-600">Batch: {alert.batch_number}</p>
                      <p className="text-xs text-gray-600 mt-1">Stock: {alert.remaining_quantity} units</p>
                    </div>
                    <div className="text-right">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">Medicines by category</p>
            </div>
          </div>
                  ...
                  <div key={idx} className="space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-gray-700">{cat.total_stock} units</span>
                      </div>
                    </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-600">Medicine inventory by location</p>
          </div>
        </div>
            ...
            {analytics.barangayDistribution.map((brgy, idx) => (
              <div key={idx} className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4 border border-blue-200">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-gray-700">Medicines:</span>
                    <span className="text-sm font-bold text-gray-900">{brgy.count}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-gray-700">Total Stock:</span>
                    <span className="text-sm font-bold text-blue-600">{brgy.total_stock.toLocaleString()} units</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6 border border-gray-200">
        <div className="text-center">
          <p className="text-sm text-gray-700 mb-2">
            📊 Data refreshed based on <strong>{timeRange === 'day' ? 'last 24 hours' : timeRange === 'week' ? 'last 7 days' : 'last 30 days'}</strong>
          </p>
