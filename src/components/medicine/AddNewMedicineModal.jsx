...
// Enable blockchain for medicine additions (same as old form)
...
// Constants (exact same as old AddMedicineForm)
...
/**
 * AddNewMedicineModal - COMPLETE VERSION WITH ALL 1050+ LINES
 * PART 1 OF 3 - Only text colors fixed to dark + enhanced error handling
 * 
 * Props:
 * - isOpen: boolean - Controls modal visibility
 * - onClose: function - Called when modal closes
 * - onSubmit: function(medicineData) - Called with medicine data when successfully added
 */
export default function AddNewMedicineModal({ isOpen, onClose, onSubmit }) {
  ...
  // Same form state as old AddMedicineForm
  ...
  // Exact same validation as old AddMedicineForm
  ...
  // Calculate unit cost based on packaging (same as old form)
  ...
  // Retry mechanism (same as old form)
  ...
  // ========== PART 2 OF 3 - SUBMIT HANDLER & BLOCKCHAIN LOGIC ==========
// This continues from Part 1

  //  ENHANCED: Submit handler with detailed error alerts
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      //  ENHANCED: Show detailed validation alert
      const errorCount = Object.keys(errors).length;
      const errorList = Object.entries(errors)
        .map(([field, msg]) => `• ${field.replace(/_/g, ' ').toUpperCase()}: ${msg}`)
        .join('\n');
      
      alert(`⚠️ Validation Errors (${errorCount})\n\n${errorList}\n\nPlease fix these errors and try again.`);
      return;
    }

    if (!address) {
      setError('Please connect your wallet first.');
      alert('⚠️ Wallet Required\n\nPlease connect your MetaMask wallet to add medicines.\n\nClick "Connect Wallet" in the top menu.');
      return;
    }

    if (!contractLoaded) {
      setError('Contract not loaded. Please refresh and try again.');
      alert('⚠️ Smart Contract Not Loaded\n\nThe blockchain smart contract is not loaded yet.\n\nPlease refresh the page and try again.');
      return;
    }
    ...
    if (ENABLE_BLOCKCHAIN_FOR_MEDICINE && !hasAccess) {
      const accessError = 
        ...
        "Your database role is: " + userRole + " Checked\n" +
    ...
    try {
      ...
      // EXACT SAME medicineData structure as old form
      const medicineData = {
      ...
      // Step 1: Create medicine in database
      console.log('Step 1: Creating medicine in database...');
      const dbResponse = await api.post('/medicines', medicineData);
      const { medicine, stock } = dbResponse.data;

      console.log('Medicine created in database:', medicine.medicine_id);

      if (ENABLE_BLOCKCHAIN_FOR_MEDICINE) {
        try {
          // Step 2: Check blockchain for duplicates
          console.log('Step 2: Checking blockchain for duplicates...');
          const existingHash = await getMedicineHash(medicine.medicine_id);

          if (existingHash && existingHash.exists) {
            console.warn('WARNING: Duplicate found on blockchain!');
            
            //  ENHANCED: Rollback with error handling
            try {
              await api.delete(`/medicines/${medicine.medicine_id}`);
              console.log('Database entry rolled back');
            } catch (rollbackErr) {
          ...
          console.log('Checked No duplicates found on blockchain');

          // Step 3: Generate hash
          console.log('Step 3: Generating blockchain hash...');
          ...
          console.log('Checked Hash generated:', dataHash.slice(0, 10) + '...');
          
          // Step 4: Store hash on blockchain with retry
          console.log('Step 4: Storing hash on blockchain with retry mechanism...');
          let txHash = null;
          ...
          try {
            ...
            console.log(' Medicine updated with blockchain info');
          } catch (updateErr) {
            console.error('⚠️ Failed to update medicine with blockchain info:', updateErr);
          }
          ...
          try {
            ...
            console.log('Stock updated with blockchain info');
          } catch (updateErr) {
            console.error('⚠️ Failed to update stock with blockchain info:', updateErr);
          }
          ...
          //  ENHANCED: Rollback on blockchain failure with detailed error handling
          console.log('Attempting to rollback database entry...');
          try {
            await api.delete(`/medicines/${medicine.medicine_id}`);
            console.log('Database entry rolled back successfully');
          } catch (rollbackErr) {
            console.error('Rollback failed:', rollbackErr);
            alert('⚠️ Critical Error\n\nBlockchain failed AND database rollback failed!\n\nMedicine ID: ' + medicine.medicine_id + '\n\nPlease contact support immediately.');
            setLoading(false);
            return;
          }
          
          //  ENHANCED: Detailed error messages for different failure types
          let errorMessage = 'Blockchain Storage Failed\n\n';
          errorMessage += 'Medicine was created in database but blockchain storage failed.\n';
          errorMessage += 'Database entry has been rolled back.\n\n';
          ...
          if (blockchainErr.message?.includes('user rejected') || 
            ...
            errorMessage += '🚫 REASON: Transaction Cancelled\n\n';
            errorMessage += 'You cancelled the MetaMask transaction.\n\n';
            errorMessage += '💡 TIP: Please try again when ready.';
          } else if (blockchainErr.message?.includes('insufficient funds')) {
            errorMessage += '💰 REASON: Insufficient MATIC\n\n';
            errorMessage += 'Your wallet does not have enough MATIC for gas fees.\n\n';
            errorMessage += '💡 TIP: Add MATIC to your wallet and try again.';
          } else if (blockchainErr.message?.includes(`failed after ${MAX_RETRIES} attempts`)) {
            errorMessage += `🌐 REASON: Network Congestion\n\n`;
            errorMessage += `Transaction failed after ${MAX_RETRIES} automatic retries.\n\n`;
            errorMessage += '💡 TIP: The blockchain network may be congested. Please try again in a few minutes.';
          } else {
            errorMessage += '🔧 REASON: Unknown Error\n\n';
            errorMessage += `Error: ${blockchainErr.message}\n\n`;
            errorMessage += '💡 TIP: Please check your wallet connection and try again.';
          }
      ...
      //  ENHANCED: Detailed error handling for different error types
      ...
      if (err.message?.includes('AccessControl')) {
        ...
        errorMessage += '💡 TIP: Contact your administrator for access.';
      } else if (err.message?.includes('already exists')) {
        errorMessage += '⚠️ REASON: Duplicate Entry\n\n';
        errorMessage += 'This batch number already exists in the system.\n\n';
        errorMessage += '💡 TIP: Please use a unique batch number.';
      } else if (err.response?.data?.error) {
        errorMessage += '🔧 REASON: Server Error\n\n';
        errorMessage += `${err.response.data.error}\n\n`;
        errorMessage += '💡 TIP: Please check the error details and try again.';
      } else if (err.message?.includes('Network Error') || err.message?.includes('timeout')) {
        errorMessage += '🌐 REASON: Network Error\n\n';
        errorMessage += 'Could not connect to the server.\n\n';
        errorMessage += '💡 TIP: Check your internet connection and try again.';
      } else {
        errorMessage += '🔧 REASON: Unknown Error\n\n';
        errorMessage += `${err.message || 'An unexpected error occurred'}\n\n`;
        errorMessage += '💡 TIP: Please try again or contact support if the problem persists.';
      }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full my-8 max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 rounded-t-xl flex items-center justify-between z-10">
          <div className="flex items-center gap-3">
            <Package className="w-8 h-8" />
            <div>
              <h2 className="text-2xl font-bold">Add New Medicine</h2>
              <p className="text-white text-sm">Complete all required fields - MetaMask confirmation required</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Retry Status -  Already dark text */}
        {loading && retryCount > 0 && (
          <div className="bg-blue-50 border-2 border-blue-400 rounded-xl p-5 m-6 shadow-md animate-pulse">
            <div className="flex items-center gap-3">
        ...
        {/* Error Message -  Already dark text */}
        {error && (
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-200">
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="flex-1 px-6 py-3 border border-gray-300 rounded-lg text-gray-900 font-medium hover:bg-gray-50 transition-colors disabled:opacity-50"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading || !address || !contractLoaded}
              className="flex-1 px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-lg font-bold transition-colors disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed"
            >
              {loading ? (
                <span className="flex items-center justify-center gap-2">
                  <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  {retryCount > 0 ? `Retrying (${retryCount}/${MAX_RETRIES})...` : 'Adding Medicine...'}
                </span>
              ) : (
                'Add Medicine (Sign with MetaMask)'
              )}
            </button>
          </div>
        </form>
...
// ========== END OF PART 3 ==========
// To use: Combine Part 1 + Part 2 + Part 3 into single file
